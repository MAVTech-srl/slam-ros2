...
if [ "$PLATFORM" = "Raspberry" ]; then # Run Raspberry image
    docker run --rm \
        ...
        ${DOCKER_ARGS[@]} \
        --name fast-lio-slam \
        ghcr.io/mavtech-srl/fast-lio-slam:0.6-rasp-dev ros2 launch --noninteractive src/slam_tools/launch/slam.launch.py \
              rviz:=$RVIZ_USE \
    ...
    docker run --rm \
        ...
        ${DOCKER_ARGS[@]} \
        --name fast-lio-slam \
        ghcr.io/mavtech-srl/fast-lio-slam:0.6-dev ros2 launch --noninteractive src/slam_tools/launch/slam.launch.py \
              rviz:=$RVIZ_USE \
              save_pcd_cloud:=$LOCAL_PCD_USE \
              save_UTM_pcd_cloud:=$UTM_PCD_USE \
              convert_livox_cloud:=$CONVERT \
